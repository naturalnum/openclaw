...
如果这次只有 `power-ui` 前端变更（没有改 `src/`、`power-backend`、依赖清单），可以走快速模式：

```bash
cd deploy
OPENCLAW_BUILD_MODE=frontend-only ./build-image.sh
```

快速模式只会重建 `power-ui`，复用当前机器已有的 `dist` 运行时产物和依赖，不再执行整套 install/build。
快速模式与 full 模式一样，都会先准备一个独立的 Docker build context；默认放在仓库根目录 `.tmp/` 下（可用 `OPENCLAW_BUILD_TMP_ROOT` 覆盖），避免系统 `/tmp` 空间不足。

注意：

- 第一次构建，或改过后端/runtime 时，必须先跑一次默认的 `OPENCLAW_BUILD_MODE=full`。
- 快速模式依赖当前目录已有 `dist/index.js` 和 `dist/extensions/power-backend/index.mjs`，缺失时脚本会直接报错并提示改回 full 模式。
...
- `OPENCLAW_NODE_IMAGE`
- `OPENCLAW_PLATFORM`
- `OPENCLAW_BUILD_MODE`（`full` 或 `frontend-only`）
- `OPENCLAW_EXTENSIONS`
