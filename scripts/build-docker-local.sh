...
NODE_IMAGE="${2:-node:24-bookworm-slim}"
PLATFORM="${3:-}"
...
if [[ ! -f "$REPO_ROOT/dist/control-ui/index.html" ]]; then
  echo "Control UI assets missing; running pnpm ui:build" >&2
  (cd "$REPO_ROOT" && pnpm ui:build)
fi

for required in "${required_assets[@]}" "$REPO_ROOT/dist/control-ui/index.html"; do
  if [[ ! -e "$required" ]]; then
    echo "Missing build asset: $required" >&2
    echo "Building Docker assets with: pnpm build:docker && pnpm ui:build" >&2
    (cd "$REPO_ROOT" && pnpm build:docker && pnpm ui:build)
    break
  fi
done
...
for required in \
  ...
  "$REPO_ROOT/dist/power-ui/index.html" \
  "$REPO_ROOT/dist/extensions/power-backend/index.mjs" \
  "$REPO_ROOT/default_config/.env"; do
  if [[ ! -e "$required" ]]; then
...
for item in dist extensions skills docs qa default_config; do
