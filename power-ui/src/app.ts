...
export class OpenClawPowerApp extends LitElement {
  ...
  private readonly dreamingState: DreamingState = {
    client: this.controllerClient,
    connected: false,
    configSnapshot: null,
  ...
  private async initializeGatewaySession() {
    ...
    this.syncDreamingControllerContext();
  ...
  private async bootstrap() {
    try {
      ...
      this.dreamingState.connected = false;
      for (const runtime of this.chatRuntimeBySessionKey.values()) {
  ...
  private applyGlobalModelSettings(snapshot: ConfigSnapshot) {
    ...
    this.dreamingState.configSnapshot = snapshot;
    const config = snapshot.config ?? {};
  ...
  private syncDreamingControllerContext() {
    this.dreamingState.client = this.controllerClient;
    this.dreamingState.connected = this.connected;
    this.dreamingState.configSnapshot = this.configSnapshot;
    this.dreamingState.applySessionKey = this.sessionKey || "main";
  }

  private async refreshConfigSnapshot() {
    const snapshot = await this.loadGlobalModelSettings();
    this.applyGlobalModelSettings(snapshot);
    return snapshot;
  }

  private async openWikiPagePreview(lookup: string) {
    if (!this.connected) {
      return null;
    }
    const payload = (await this.controllerClient.request("wiki.get", {
      lookup,
      fromLine: 1,
      lineCount: 5000,
    })) as {
  ...
  private async loadDreamingPage() {
    await this.runControllerAction(
      (async () => {
        await this.refreshConfigSnapshot();
        this.syncDreamingControllerContext();
        const memoryCoreEnabled = isPluginExplicitlyEnabled(this.configSnapshot, "memory-core");
        const memoryWikiEnabled = isPluginExplicitlyEnabled(this.configSnapshot, "memory-wiki");
        const tasks: Promise<unknown>[] = [];
        if (memoryCoreEnabled) {
          tasks.push(loadDreamingStatus(this.dreamingState), loadDreamDiary(this.dreamingState));
        }
        if (memoryWikiEnabled) {
          tasks.push(
            loadWikiImportInsights(this.dreamingState),
            loadWikiMemoryPalace(this.dreamingState),
          );
        } else {
          this.dreamingState.wikiImportInsightsError = null;
          this.dreamingState.wikiImportInsights = null;
          this.dreamingState.wikiMemoryPalaceError = null;
          this.dreamingState.wikiMemoryPalace = null;
        }
        await Promise.all(tasks);
      })(),
    );
    if (this.dreamingState.lastError) {
      this.lastError = this.dreamingState.lastError;
    }
  }
  ...
  private renderView() {
    ...
    this.syncDreamingControllerContext();
    const configuredDreaming = resolveConfiguredDreaming(
      (this.configSnapshot?.config as Record<string, unknown> | null) ?? null,
    );
    const dreamingOn = this.dreamingState.dreamingStatus?.enabled ?? configuredDreaming.enabled;
    const dreamingLoading =
      this.dreamingState.dreamingStatusLoading || this.dreamingState.dreamingModeSaving;
    const dreamingRefreshLoading =
      this.dreamingState.dreamingStatusLoading || this.dreamingState.dreamDiaryLoading;
    ...
    return renderWorkbench({
      ...
      skillsPage: {
        ...
        onCategoryChange: (next) => {
          void this.runControllerAction(setSkillsCategory(this.skillsState, next));
        },
        onSortChange: (next) => {
          void this.runControllerAction(setSkillsSortBy(this.skillsState, next));
        },
        onInstallFilterChange: (next) => {
          void this.runControllerAction(setSkillsInstallFilter(this.skillsState, next));
        },
        onRefresh: () => {
          void this.loadSkillsPage(true);
        },
        onPageChange: (next) => {
          void this.runControllerAction(setSkillsPage(this.skillsState, next));
        },
      ...
      dreamingPage: {
        active: dreamingOn,
        loading: dreamingLoading,
        refreshLoading: dreamingRefreshLoading,
        view: {
          active: dreamingOn,
          shortTermCount: this.dreamingState.dreamingStatus?.shortTermCount ?? 0,
          groundedSignalCount: this.dreamingState.dreamingStatus?.groundedSignalCount ?? 0,
          totalSignalCount: this.dreamingState.dreamingStatus?.totalSignalCount ?? 0,
          promotedCount: this.dreamingState.dreamingStatus?.promotedToday ?? 0,
          phases: this.dreamingState.dreamingStatus?.phases ?? undefined,
          shortTermEntries: this.dreamingState.dreamingStatus?.shortTermEntries ?? [],
          promotedEntries: this.dreamingState.dreamingStatus?.promotedEntries ?? [],
          dreamingOf: null,
          nextCycle: dreamingNextCycle,
          timezone: this.dreamingState.dreamingStatus?.timezone ?? null,
          statusLoading: this.dreamingState.dreamingStatusLoading,
          statusError: this.dreamingState.dreamingStatusError,
          modeSaving: this.dreamingState.dreamingModeSaving,
          dreamDiaryLoading: this.dreamingState.dreamDiaryLoading,
          dreamDiaryActionLoading: this.dreamingState.dreamDiaryActionLoading,
          dreamDiaryActionMessage: this.dreamingState.dreamDiaryActionMessage,
          dreamDiaryActionArchivePath: this.dreamingState.dreamDiaryActionArchivePath,
          dreamDiaryError: this.dreamingState.dreamDiaryError,
          dreamDiaryPath: this.dreamingState.dreamDiaryPath,
          dreamDiaryContent: this.dreamingState.dreamDiaryContent,
          memoryWikiEnabled,
          wikiImportInsightsLoading: this.dreamingState.wikiImportInsightsLoading,
          wikiImportInsightsError: this.dreamingState.wikiImportInsightsError,
          wikiImportInsights: this.dreamingState.wikiImportInsights,
          wikiMemoryPalaceLoading: this.dreamingState.wikiMemoryPalaceLoading,
          wikiMemoryPalaceError: this.dreamingState.wikiMemoryPalaceError,
          wikiMemoryPalace: this.dreamingState.wikiMemoryPalace,
          onRefresh: () => {
            void this.loadDreamingPage();
          },
          onRefreshDiary: () => {
            this.syncDreamingControllerContext();
            void this.runControllerAction(loadDreamDiary(this.dreamingState));
          },
          onRefreshImports: () => {
            this.syncDreamingControllerContext();
            void this.runControllerAction(loadWikiImportInsights(this.dreamingState));
          },
          onRefreshMemoryPalace: () => {
            this.syncDreamingControllerContext();
            void this.runControllerAction(loadWikiMemoryPalace(this.dreamingState));
          },
          onOpenConfig: () => {
            window.location.href = `${this.basePath}/config`;
          },
          onOpenWikiPage: (lookup: string) => this.openWikiPagePreview(lookup),
          onBackfillDiary: () => {
            this.syncDreamingControllerContext();
            void this.runControllerAction(backfillDreamDiary(this.dreamingState));
          },
          onCopyDreamingArchivePath: () => {
            this.syncDreamingControllerContext();
            void this.runControllerAction(copyDreamingArchivePath(this.dreamingState));
          },
          onDedupeDreamDiary: () => {
            this.syncDreamingControllerContext();
            void this.runControllerAction(dedupeDreamDiary(this.dreamingState));
          },
          onResetDiary: () => {
            this.syncDreamingControllerContext();
            void this.runControllerAction(resetDreamDiary(this.dreamingState));
          },
          onResetGroundedShortTerm: () => {
            this.syncDreamingControllerContext();
            void this.runControllerAction(resetGroundedShortTerm(this.dreamingState));
          },
          onRepairDreamingArtifacts: () => {
            this.syncDreamingControllerContext();
            void this.runControllerAction(repairDreamingArtifacts(this.dreamingState));
          },
          onRequestUpdate: () => {
            this.requestUpdate();
          },
        ...
        onToggleEnabled: () => {
          ...
          void this.runControllerAction(
            (async () => {
              this.syncDreamingControllerContext();
              const updated = await updateDreamingEnabled(this.dreamingState, !dreamingOn);
              if (!updated) {
                return;
              }
              await this.refreshConfigSnapshot();
              this.syncDreamingControllerContext();
              await loadDreamingStatus(this.dreamingState);
            })(),
          );
        },
      },
