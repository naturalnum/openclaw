...
def toc_display_text(heading: str, level: int) -> str:
    if level == 2:
        ...
        heading = heading.replace("面向配网检修运行方式动态协调编排的", "")
        heading = heading.replace("基于先验知识内嵌机器学习的", "")
        heading = heading.replace("检修计划与运行方式预案的", "")
        heading = heading.replace("人机协同数据知识增强的配电系统", "人机协同的配电系统")
        heading = heading.replace("智能校核与编制优化模块的开发与应用", "智能校核与编制优化模块")
        heading = heading.replace("研究多时间尺度检修计划编排和运行方式调整的", "")
        heading = heading.replace("耦合机制与联合建模方法", "联合建模方法")
        heading = heading.replace("研究检修计划报送-校核-编排及运行方式调整的", "")
        heading = heading.replace("先验知识提取与表征方法", "知识提取与表征方法")
        heading = heading.replace("研究配网多时间尺度多电压等级检修运行方式动态协调编排的", "")
        heading = heading.replace("人机数据知识融合增强架构", "数据知识融合增强架构")
        heading = heading.replace("研究基于先验知识内嵌机器学习的", "")
        heading = heading.replace("配网运行安全约束建模方法", "安全约束建模方法")
        heading = heading.replace("研究基于多粒度时空学习的", "")
        heading = heading.replace("配网检修风险等级预测技术", "检修风险等级预测技术")
        heading = heading.replace("研究数据驱动与知识逻辑校验联合驱动的", "")
        heading = heading.replace("风险主动发现与溯因分析方法", "风险主动发现与溯因方法")
        heading = heading.replace("检修计划与运行方式预案的专家反馈趋优学习与知识更新技术研究", "专家反馈趋优学习与知识更新")
        heading = heading.replace("人机协同数据知识增强的配电系统检修运行方式智能校核与编制优化模块的开发与应用", "人机协同智能校核与编制优化模块")
        heading = heading.replace("研究考虑运行规则、安全风险、检修工作量等多目标因素的", "")
        heading = heading.replace("计划编排与方式预案生成智能体构建技术", "预案生成智能体构建")
        heading = heading.replace("设计检修运行方式智能校核与编制优化模块的", "")
        heading = heading.replace("基础架构、交互方式与集成方案", "基础架构与集成方案")
        heading = heading.replace("在实际地区电网对", "")
        heading = heading.replace("检修运行方式智能校核与编制优化模块开展试点应用验证", "智能校核与编制优化模块试点验证")
    if level == 3:
    ...
    if level == 1:
        ...
        heading = heading.replace("人机协同数据知识增强的配电系统", "人机协同的配电系统")
        heading = heading.replace("智能校核与编制优化模块的开发与应用", "智能校核与编制优化模块")
    heading = heading.replace("技术研究", "研究")
    heading = heading.replace("方法研究", "方法")
    heading = heading.replace("机制研究", "机制")
    return heading
